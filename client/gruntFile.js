module.exports = function (grunt) {
  ...
  // Default task.
  grunt.registerTask('default', ['karma:unit','build' /*,'release'*/]);
  grunt.registerTask('build', ['clean','concat','copy:assets']);
  grunt.registerTask('release', ['clean','jshint', 'karma:unit', 'build', /*'copy:hosting', 'shell:parsedeploy'*/]);
  ...
  grunt.initConfig({
    distdir: 'dist',
    hostingdir: '../server/angular-template/public',
    ...
    shell:{ 
      parsedeploy: {
        command: ['cd ../server/angular-template', 'parse deploy'].join('&&')
      }
    },
    concat:{
      dist:{
      ...
      angular: {
        src:['bower_components/angular/angular.js', 'bower_components/angular-route/angular-route.js', 'bower_components/angular-cookies/angular-cookies.js', 'bower_components/angular-resource/angular-resource.js'],
    ...
    watch:{
      all: {
        files:['<%= src.js %>', '<%= src.fixtures %>', '<%= src.lessWatch %>', '<%= src.tpl.app %>', '<%= src.tpl.common %>', '<%= src.html %>'],
